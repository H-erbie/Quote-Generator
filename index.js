...
var card = document.querySelector('.quotes');
var random = document.querySelector('.random');
var linear = document.querySelector('.linear')
var quo = document.createElement('div');
quo.setAttribute('class', 'quote');
var aut = document.createElement('div');
quo.setAttribute('class', 'author');
var ranMode = document.querySelector('.ran-mode')
var info = document.querySelector('.info');
var index = 0;

    var qdiv = document.getElementsByClassName('carDiv');


linear.addEventListener('click', () => {
   show_quote(index)
})
   function show_quote (i) {
    document.querySelector('.but-con').style.display = 'none';
    document.querySelector('.but-con').style.display = 'flex';
    quo.innerHTML = '';
    aut.innerHTML = '';
    ranMode.style.display = 'none';
    card.innerHTML = ''
    ...
    for (let quote in quotes){
        ...
        div.innerHTML = `${quotes[quote]} <br> <br> ${quote}`;
        ...
        if(index > qdiv.length - 1)
            index = 0;
        if(index < 0)
            index = qdiv.   length - 1;
        qdiv[index].style.display = 'block'
    }


random.addEventListener('click',  () => {   
    info.style.display = 'none'
    document.querySelector('.ran-mode').style.display = 'flex'; 
    document.querySelector('.but-con').style.display = 'none';
     quo.innerHTML = 'click on "generate" to start';
     card.innerHTML = quo.innerHTML;
    ...
    document.querySelector('.ran-mode').addEventListener('click', () => {
